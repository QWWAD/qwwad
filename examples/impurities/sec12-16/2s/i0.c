...
double	y1,y2=0,y3;	/* E(lambda-), E(lambda), E(lambda+)	*/
double	Y2;		/* previous value of y2			*/
...
while((argc>1)&&(argv[1][0]=='-'))
{
 ...
 argc--;
  ...
  while(fscanf(fr_i,"%lf\n",&r_i)!=EOF)
  {
   ...
   do
   {
    Y2=y2;			/* remember energy for next pass	*/
   ...
   //}while((fabs(f/fdash)>1e-10)&&(fabs(Y2-y2)>(1e-3*e_0/10)));	
