 ...
 switch(argv[1][1])
 {
  ...
  case 'D':
	   ...
	   switch(form_of_D)
	   {
	    ...
	    case 2: break;
	    case 3: break;
	    default: printf("Usage: gde [-D form of diffusion coefficient (\033[1m1\033[0m, 2 or 3)\n");
		     printf("       \033[1m1\033[0m=>D=D(z) only, read in from file D.r\n");
  ...
  case 't':
	   ...
	   break;
  default:
	   printf("Usage: gde [-D form of diffusion coefficient (\033[1m1\033[0m, 2 or 3)\n");
...
switch(form_of_D)
{
 ...
 case 2:
        for(t=delta_t;t<=t_final;t+=delta_t)
        {
         calculate_D(data_start,D_start,t,n);
         diffuse(data_start,D_start,FX,delta_t,delta_z,n);
        }break;
 case 3:
        read_D(D_start);
