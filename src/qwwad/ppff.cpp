...
auto Vf_ALASmz(double A0,
               double m_per_au,
               double q_sqr) -> double
{
    q_sqr/=gsl_pow_2(2*pi/A0);        // convert q from SI into units of (2*pi/A0)
    double A0_au=A0/m_per_au;	      // convert A0 from S.I.-->a.u.
    q_sqr*=gsl_pow_2(2*pi/A0_au);     // convert q into atomic units
    double Omega=A0_au*A0_au*A0_au/4; // atomic volume, i.e. fcc cube/4

    // Find the atomic potential
    double Va=(-1.32712*exp(-1.59819*gsl_pow_2(sqrt(q_sqr)-0))
               ...
               double q_sqr) -> double
{
    ...
    // Find the atomic potential
    double Va=(-1.24498*exp(-1.52748*gsl_pow_2(sqrt(q_sqr)-0))
               ...
               double q_sqr) -> double
{
    ...
    // Find the atomic potential
    double Va=(-1.10411*exp(-0.972439*gsl_pow_2(sqrt(q_sqr)-0))
               ...
               double q_sqr) -> double
{
    ...
    // Find the atomic potential
    double Va=(-1.0582*exp(-0.959327*gsl_pow_2(sqrt(q_sqr)-0))
        ...
        const char   *type) -> double
{
    ...
    if(strcmp(type,"SI") == 0) {
        ...
        value = 2*Va*h*c*Rinf;	/*  first factor a.u.--> Rydberg */
        ...
        value = Vf_GAASmz(A0, m_per_au, q_sqr);
    } else if(strcmp(type,"ASGAmz") == 0) {
        value = Vf_ASGAmz(A0, m_per_au, q_sqr);
    } else if(strcmp(type,"ALASmz") == 0) {
        value = Vf_ALASmz(A0, m_per_au, q_sqr);
    } else if(strcmp(type,"ASALmz") == 0) {
        value = Vf_ASALmz(A0, m_per_au, q_sqr);
    } else if(strncmp(type,"ALGAAS",6) == 0) {
        x=(10*(double)(*(type+6)-48)+(double)(*(type+7)-48))/100;

        value = x*Vf_ALASmz(A0,m_per_au,q_sqr)+(1-x)*Vf_GAASmz(A0,m_per_au,q_sqr);
    } else if(strncmp(type,"ASALGA",6) == 0) {
        x=(10*(double)(*(type+6)-48)+(double)(*(type+7)-48))/100;

        value = x*Vf_ASALmz(A0,m_per_au,q_sqr)+(1-x)*Vf_ASGAmz(A0,m_per_au,q_sqr);
    }
    ...
    /* These atomic potentials are derived from the Symmetric and
       ...
       been deduced by linear interpolation(!) from the existing data.	*/
        ...
        value = Va*e;	/* Convert eV --> SI	*/
    } else if(strcmp(type,"INAScb") == 0) { /* In in InAs	*/
        q_sqr/=gsl_pow_2(2*pi/A0);		/* convert q from SI into units of (2*pi/A0) */
        Va=-2.04*(Theta(q_sqr-2.9)-Theta(q_sqr-3.1))		/* Vf(sqrt(3))	*/
        ...
        value = Va*e;	/* Convert eV --> SI	*/
    } else if(strcmp(type,"ASINcb") == 0) { /* As in InAs	*/
        q_sqr/=gsl_pow_2(2*pi/A0);		/* convert q from SI into units of (2*pi/A0) */
        ...
        value = Va*e;	/* Convert eV --> SI	*/
    } else if(strcmp(type,"GAPcb") == 0) { /* Ga in GaP	*/
        q_sqr/=gsl_pow_2(2*pi/A0);		/* convert q from SI into units of (2*pi/A0) */
        ...
        value = Va*e;	/* Convert eV --> SI	*/
    } else if(strcmp(type,"PGAcb") == 0) {/* P in GaP	*/
        q_sqr/=gsl_pow_2(2*pi/A0);		/* convert q from SI into units of (2*pi/A0) */
        ...
        value = Va*h*c*Rinf;	/* Convert Rydberg --> SI */
    } else if(strcmp(type,"TECDcb") == 0) { /* Te in CdTe	*/
        q_sqr/=gsl_pow_2(2*pi/A0);		/* convert q from SI into units of (2*pi/A0) */
    ...
    /* Chelikowsky and Cohen, Phys. Rev. B14 p556 (1976)	*/
    ...
    else if(strcmp(type,"GAAScc") == 0) { /* Ga in GaAs	*/
        q_sqr/=gsl_pow_2(2*pi/A0);		/* convert q from SI into units of (2*pi/A0) */
        ...
        value = Va*h*c*Rinf;	/* Convert Rydberg --> SI	*/
    } else if(strcmp(type,"ASGAcc") == 0) { /* As in GaAs	*/
        q_sqr/=gsl_pow_2(2*pi/A0);		/* convert q from SI into units of (2*pi/A0) */
     ...
     * p6939 (1996)							*/
    ...
    else if(strcmp(type,"CDTE") == 0) { /* Cd in CdTe	*/
        q_sqr/=gsl_pow_2(2*pi/A0);		/* convert q from SI into units of (2*pi/A0) */
        Va=-0.175*(Theta(q_sqr-2.9)-Theta(q_sqr-3.1))		/* Vf(sqrt(3))	*/
        ...
        value = Va*h*c*Rinf;	/* Convert Rydberg --> SI */
    } else if(strcmp(type,"TECD") == 0) { /* Te in CdTe	*/
        q_sqr/=gsl_pow_2(2*pi/A0);		/* convert q from SI into units of (2*pi/A0) */
        ...
        oss << "Error atom type '" << type << "' undefined!" << std::endl;
